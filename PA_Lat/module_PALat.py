...
def axis(agent, *larg) :
    ...
    if fcu_lat_vector[0] == 'SelectedHeading' :
        capture_cap((float(fcu_lat_vector[1])-magnetic_Declinaison_vector[0])*deg2rad)
...
def ground_speed():
    ...
    x_dot =state_vector[3]*np.cos(state_vector[4])*np.cos(state_vector[5]) - wind_vector[0]*np.cos(wind_vector[1]*deg2rad)
    y_dot = state_vector[3]*np.cos(state_vector[4])*np.sin(state_vector[5])  - wind_vector[0]*np.sin(wind_vector[1]*deg2rad)
    return ((x_dot)**2+ (y_dot)**2)**0.5
    

## derive
def derive():
    X = np.arctan2(state_vector[3]*np.sin(state_vector[5])+wind_vector[0]*np.sin(wind_vector[1]*deg2rad+np.pi),state_vector[3]*np.cos(state_vector[5])+wind_vector[0]*np.cos(wind_vector[1]*deg2rad+np.pi))
    a = X - wind_vector[1]*deg2rad
    sind = wind_vector[0]*np.sin(a)/state_vector[3]
    if sind > 1 :
        sind = 1
    elif sind < -1:
        sind = -1
